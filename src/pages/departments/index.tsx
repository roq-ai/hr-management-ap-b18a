...
import { getDepartments, deleteDepartmentById } from 'apiSdk/departments';
import { DepartmentInterface } from 'interfaces/department';

type ColumnType = ColumnDef<DepartmentInterface, unknown>;
...
export function DepartmentListPage(props: DepartmentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getDepartments({
        relations: ['company', 'employee', 'project.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DepartmentInterface>>(
    () => `/departments?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DepartmentInterface) => {
    if (hasAccess('department', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/departments/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/departments/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('department', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/departments/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('department', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="department" />
        </Flex>

        {hasAccess('department', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/departments/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
