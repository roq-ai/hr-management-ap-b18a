...
import { getAttendances } from 'apiSdk/attendances';
...
function DashboardPage() {
  ...
  const attendanceStat = useSWR<PaginatedInterface<AttendanceInterface>>(
    () => `/attendances`,
    () => getAttendances({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Attendances',
      result: attendanceStat?.data?.totalCount,
      isLoading: attendanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
